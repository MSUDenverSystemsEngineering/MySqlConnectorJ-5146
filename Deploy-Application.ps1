...
Try {
	...
	[string]$appName = 'MySQL ConnectorJ'
	[string]$appVersion = '5.1.46'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		If ((Test-Path $envCommonDocuments\mysql-connector-java-5.1.46) -eq $false) {
				Copy-File -Path "$dirFiles\mysql-connector-java-5.1.46" -Destination "$envCommonDocuments" -Recurse
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		# <Perform Uninstallation tasks here>
			If ((Test-Path $envCommonDocuments\mysql-connector-java-5.1.46) -eq $true) {
				Remove-File -Path "$envCommonDocuments\mysql-connector-java-5.1.46" -Recurse
			}
